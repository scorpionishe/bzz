...
- **Enter keeps its modifiers.** Shift+Enter / Option+Enter after a corrected word are re-sent as pressed, so Shift+Enter in Telegram or Slack stays a line break; Enter is intercepted only when a fix actually exists.
- **Boundary characters survive a correction.** `превет-пока` keeps its hyphen, `(превет)` its bracket — the character that ended the word is retyped instead of a space, for spelling and layout fixes alike.
...
- **Новая иконка приложения.** Белая надпись `bzz` на плитке с сине-фиолетовым градиентом; по-прежнему генерируется как векторная графика при сборке (`scripts/gen_icon.py`), поэтому чёткая на любом размере.
...
- **Орфография перестала "исправлять" правильные слова.** На выборке текстов автора 83% автозамен были ложными: словоформы, которых нет в словаре macOS (`трафика` → `трафик`, `виртуальной` → `виртуально`), имена (`Димке` → `Диске`), англицизмы (`бакет` → `пакет`). Слово, чью лемму система принимает, больше не правится добавлением или удалением буквы, формы имён с большой буквы не трогаются, наречия на `-нно`/`-но` и `ё` никогда не "исправляются", а `dicts/ru_extra.txt` пополнен ~110 IT-терминами. Имена собственные стали кандидатами (`Масква` → `Москва`, `Рассия` → `Россия`), а коллизии коротких основ не выбирают несуществующее слово (`фразо` → `фраза`, `потак` → `поток`). Ложных автозамен на выборке стало 35 вместо 239, настоящие опечатки исправляются по-прежнему.
- **Enter сохраняет модификаторы.** Shift+Enter / Option+Enter после исправленного слова пересылаются как нажаты, так что Shift+Enter в Telegram или Slack остаётся переносом строки; Enter перехватывается только при найденной правке.
- **Граничный символ переживает исправление.** `превет-пока` сохраняет дефис, `(превет)` скобку: символ, завершивший слово, набирается заново вместо пробела, и для орфографии, и для раскладки.
